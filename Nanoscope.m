classdef Nanoscope
    ...
    methods
        ...
        function obj = set.phaseMaskPara(obj, s)
            ...
            if nargin > 0
                ...
                if isfield(s,'calibrated_mask')
                    s_t.calibrated_mask = s.calibrated_mask;
                end
    ...
    methods (Static)
        ...
        function [imgx, imgy, Ex, Ey] = simDipole_novotny(Nanoscope, Emitter, varargin)
            ...
            pmask_x = pmask(:, :, 1);
            pmask_y = pmask(:, :, 2);
            if isfield(Nanoscope.phaseMaskPara,'calibrated_mask')
                load(Nanoscope.phaseMaskPara.calibrated_mask);
                pmask_x = exp(1j*pmaskX);
                pmask_y = exp(1j*pmaskY);
            end
        ...
        function [PSFx, PSFy] = createPSFstruct3D(obj, varargin)
            ...
            if isfield(s, 'axial_grid_points')
                ...
                axial_grid_points = s.axial_grid_points; %in unit of nm
                deltaz = abs(axial_grid_points(2)-axial_grid_points(1))/2;
            else
                axial_grid_points = 0;
            ...
            if isfield(s, 'scaling_factor')

                scaling_factor = s.scaling_factor; %in unit of m
            else
                scaling_factor = [100,100,100];
            ...
            for ii = 1:length(z_position)
                ...
                Emitter.position_para.z2 = z_cur+deltaz;
                ...
                imgXXxdy = (imgxdy2 - imgxdy1) / deltax;
                imgXXydy = (imgydy2 - imgydy1) / deltax;
                imgXXxdz = (imgxdz2 - imgxdz1) / deltaz;
                imgXXydz = (imgydz2 - imgydz1) / deltaz;
                %YY
                %---------------------------------------------------------
                %set molecular orientation
                Emitter.polar_para.phiD = pi / 2;
                ...
                imgYYxdy = (imgxdy2 - imgxdy1) / deltax;
                imgYYydy = (imgydy2 - imgydy1) / deltax;
                imgYYxdz = (imgxdz2 - imgxdz1) / deltaz;
                imgYYydz = (imgydz2 - imgydz1) / deltaz;
                ...
                imgZZxdy = (imgxdy2 - imgxdy1) / deltax;
                imgZZydy = (imgydy2 - imgydy1) / deltax;
                imgZZxdz = (imgxdz2 - imgxdz1) / deltaz;
                imgZZydz = (imgydz2 - imgydz1) / deltaz;
                ...
                if ii==1
                    ...
                    %gradients
                    PSFx.XXxdx = single(scaling_factor(1)* imgXXxdx);
                    PSFx.XXxdy = single(scaling_factor(2) * imgXXxdy);
                    PSFx.XXxdz = single(scaling_factor(3) * imgXXxdz);
                    PSFx.YYxdx = single(scaling_factor(1)* imgYYxdx);
                    PSFx.YYxdy = single(scaling_factor(2)* imgYYxdy);
                    PSFx.YYxdz = single(scaling_factor(3) * imgYYxdz);                
                    PSFx.ZZxdx = single(scaling_factor(1) * imgZZxdx);
                    PSFx.ZZxdy = single(scaling_factor(2) * imgZZxdy);
                    PSFx.ZZxdz = single(scaling_factor(3) * imgZZxdz);
                    ...
                    %gradients
                    PSFy.XXydx = single(scaling_factor(1) * imgXXydx);
                    PSFy.XXydy = single(scaling_factor(2) * imgXXydy);
                    PSFy.XXydz = single(scaling_factor(3) * imgXXydz);
                    PSFy.YYydx = single(scaling_factor(1) * imgYYydx);
                    PSFy.YYydy = single(scaling_factor(2) * imgYYydy);
                    PSFy.YYydz = single(scaling_factor(3) * imgYYydz);
                    PSFy.ZZydx = single(scaling_factor(1) * imgZZydx);
                    PSFy.ZZydy = single(scaling_factor(2) * imgZZydy);
                    PSFy.ZZydz = single(scaling_factor(3) * imgZZydz);
                    ...
                    %gradients
                    PSFx.XXxdx = cat(3,PSFx.XXxdx,single(scaling_factor(1) * imgXXxdx));
                    PSFx.XXxdy = cat(3,PSFx.XXxdy,single(scaling_factor(2) * imgXXxdy));
                    PSFx.XXxdz = cat(3,PSFx.XXxdz,single(scaling_factor(3) * imgXXxdz));
                    PSFx.YYxdx = cat(3,PSFx.YYxdx,single(scaling_factor(1) * imgYYxdx));
                    PSFx.YYxdy = cat(3,PSFx.YYxdy,single(scaling_factor(2) * imgYYxdy));
                    PSFx.YYxdz = cat(3,PSFx.YYxdz,single(scaling_factor(3) * imgYYxdz));                
                    PSFx.ZZxdx = cat(3,PSFx.ZZxdx,single(scaling_factor(1) * imgZZxdx));
                    PSFx.ZZxdy = cat(3,PSFx.ZZxdy,single(scaling_factor(2) * imgZZxdy));
                    PSFx.ZZxdz = cat(3,PSFx.ZZxdz,single(scaling_factor(3) * imgZZxdz));
                    ...
                    %gradients
                    PSFy.XXydx = cat(3,PSFy.XXydx,single(scaling_factor(1) * imgXXydx));
                    PSFy.XXydy = cat(3,PSFy.XXydy,single(scaling_factor(2) * imgXXydy));
                    PSFy.XXydz = cat(3,PSFy.XXydz,single(scaling_factor(3) * imgXXydz));
                    PSFy.YYydx = cat(3,PSFy.YYydx,single(scaling_factor(1) * imgYYydx));
                    PSFy.YYydy = cat(3,PSFy.YYydy,single(scaling_factor(2) * imgYYydy));
                    PSFy.YYydz = cat(3,PSFy.YYydz,single(scaling_factor(3) * imgYYydz));
                    PSFy.ZZydx = cat(3,PSFy.ZZydx,single(scaling_factor(1) * imgZZydx));
                    PSFy.ZZydy = cat(3,PSFy.ZZydy,single(scaling_factor(2) * imgZZydy));
                    PSFy.ZZydz = cat(3,PSFy.ZZydz,single(scaling_factor(3) * imgZZydz));
